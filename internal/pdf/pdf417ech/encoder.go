...
func EncodeWithColor(data string, securityLevel byte, color barcode.ColorScheme) (barcode.Barcode, error) {
	...
	if len(dataWords)+1+sl.ErrorCorrectionWordCount() > columns*rows {
		return nil, fmt.Errorf("unable to fit %d data codewords in %dx%d PDF417 symbol", len(dataWords), columns, rows)
	}
...
func encodeCodewords(data string, dataWords []int, securityLevel byte, color barcode.ColorScheme) (barcode.Barcode, error) {
	sl := securitylevel(securityLevel)
	columns, rows := echColumns, echRows
	if len(dataWords)+1+sl.ErrorCorrectionWordCount() > columns*rows {
		return nil, fmt.Errorf("unable to fit %d data codewords in %dx%d PDF417 symbol", len(dataWords), columns, rows)
	}
	...
	if len(codeWords) != columns*rows {
		return nil, fmt.Errorf("internal error: rendered %d codewords, want %d", len(codeWords), columns*rows)
	}

	grid := [][]int{}
	for i := 0; i < len(codeWords); i += columns {
		grid = append(grid, codeWords[i:i+columns])
	}
...
func getPadding(dataCount int, ecCount int, columns int) []int {
	padCount := columns*echRows - dataCount - ecCount - 1
	if padCount < 0 {
		padCount = 0
	}
